// proc_HRTEC.cpp : 此文件包含 "main" 函数。程序执行将在此处开始并结束。
//
#include "stdafx.h"
...
int main(int argc, char** argv)
{
	...
	const int ans = proc_hrtec::process_hrtec(in_file, out_dir, is_debug);
	assert(ans == EXIT_SUCCESS);
	return 0;
}

bool proc_hrtec::process_command_line(int argc, char** argv,
	std::string& in_file,
	std::string& out_dir, bool& debug, bool& has_err)
{
	try
	{
		...
		desc.add_options()
			...
			("input-file,i", boost::program_options::value<std::string>(&in_file)->required(), "input HRTEC file.")
		...
		if (vm.count("version"))
		{
			std::cout << "HRTEC preprocess program version 1.0\n" << desc << "\n";
			return false;
		}
		if (vm["debug"].as<bool>())
		{
...
int proc_hrtec::process_hrtec(const std::string& in_file, const std::string& out_dir, bool is_debug)
{
	const std::filesystem::path dplg_file_path(in_file);
	const std::filesystem::path output_folder_path(out_dir);
	BOOST_LOG_TRIVIAL(debug) << "HRTEC file: " << dplg_file_path;
	BOOST_LOG_TRIVIAL(debug) << "OutputPath: " << output_folder_path;
	const std::vector<HrtecItem> items = HrtecItem::load_items(dplg_file_path.string());
	assert(!items.empty());
	const double ndv = 0.0;
	const size_t bn = 1;
	for (auto p = items.cbegin(); p != items.cend() - 1; ++p)
	{
		const HrtecItem&  item = *p;
		...
		std::string file_name{ (boost::format("HRTEC_%1%_%2$03d_%3$02d.tif") % d.year() % d.day_of_year() % item.hour()).str() };
