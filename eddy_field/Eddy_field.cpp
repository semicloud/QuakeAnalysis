...
void Eddy_field::compute_eddy_field_m1(Eddy_field_options_yaml& options)
{
	...
	const path input_image_file = options.input_image_file();
	if (!exists(input_image_file) || !is_regular_file(input_image_file))
	{
		BOOST_LOG_TRIVIAL(error) << "目标tif文件" << input_image_file << "不存在或不是常规文件，无法计算涡度！";
		BOOST_LOG_TRIVIAL(info) << "=============使用方法1计算涡度结束====================";
		BOOST_LOG_TRIVIAL(info) << "";
		return;
	}
	BOOST_LOG_TRIVIAL(info) << "目标tif文件：" << input_image_file;

	//const string ref_tif_path = options.get_ref_tif_path(type);
	const path ref_image_file = options.ref_image_file();
	if (!exists(ref_image_file) || !is_regular_file(ref_image_file))
	{
		BOOST_LOG_TRIVIAL(error) << "背景场文件" << ref_image_file << "不存在或不是常规文件，无法计算涡度！";
		BOOST_LOG_TRIVIAL(info) << "=============使用方法1计算涡度结束====================";
		BOOST_LOG_TRIVIAL(info) << "";
		return;
	}
	BOOST_LOG_TRIVIAL(info) << "背景场tif文件：" << ref_image_file;

	auto bt_mat = modis_api::Gdal_operation::read_tif_to_fmat(input_image_file.string());
	auto ref_mat = modis_api::Gdal_operation::read_tif_to_fmat(ref_image_file.string());
	...
	if (bt_mat->n_rows != ref_mat->n_rows || bt_mat->n_cols != ref_mat->n_cols)
	{
		BOOST_LOG_TRIVIAL(error) << "矩阵大小不同，无法进行计算..";
		BOOST_LOG_TRIVIAL(error) << str(boost::format("Mat1[%1%, %2%] Mat2[%3%, %4%]") % bt_mat->n_rows % bt_mat->n_cols % ref_mat->n_rows % ref_mat->n_cols);
		return;
	}

	arma::fmat diff_mat = (*bt_mat) - (*ref_mat);
	...
	// 涡度tif路径
	const path output_ano_file = options.output_ano_file();

	if (exists(output_ano_file))
		remove(output_ano_file);
	if (!exists(output_ano_file.parent_path()))
		create_directories(output_ano_file.parent_path());
	// 复制过去一个tif文件
	copy_file(input_image_file, output_ano_file);
	// 写入数据，齐活
	modis_api::Gdal_operation::write_fmat_to_tif(output_ano_file.string(), *ef_mat);
...
void Eddy_field::compute_eddy_field_m2(Eddy_field_options_yaml& options)
{
	...
	const path ef_tif_path(options.output_ano_file());
