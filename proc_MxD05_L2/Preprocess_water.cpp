...
void proc_MxD05_L2::Preprocess_water::preprocess(const std::string& yml_path, const YAML::Node& node, bool debug_mode)
{
	...
	const path hdf_file_list_file_path(node["HDFListFile"].as<std::string>());
	if (!exists(hdf_file_list_file_path))
	{
	...
	const path output_image_file(node["OutputImageFile"].as<std::string>()); //final output
	const path temp_dir(node["TmpPath"].as<std::string>());
	...
	std::vector<path> preprocessed_file_paths;
	for (const std::string& hdf_file : hdf_files)
	{
		const path hdf_file_path(hdf_file);
		...
		//Heg提取出的.tif文件路径
		const path heg_tif_path = temp_dir / (hdf_file_path.stem().string() + "_heg.tif");
		...
		const path heg_gdal_tif_path = temp_dir / (hdf_file_path.stem().string() + "_heg_gdal.tif");
		...
		const path heg_gdal_scaled_tif_path = temp_dir / (hdf_file_path.stem().string() + "_heg_gdal_scaled.tif");
	...
	for (const path& p : preprocessed_file_paths)
		BOOST_LOG_TRIVIAL(debug) << "\t" << p;
	...
	std::transform(preprocessed_file_paths.cbegin(), preprocessed_file_paths.cend(), back_inserter(mat_list),
		[](const path& p) { return *modis_api::Gdal_operation::read_tif_to_fmat(p.string());  });
