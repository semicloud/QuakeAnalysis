...
{
	std::filesystem::path tmp_folder_path(hp.temp_folder());
	...
	try
	{
		...
		BOOST_LOG_TRIVIAL(info) << prm_str;
		//保存prm文件
		const std::string prm_path = (tmp_folder_path / (hp.input_file().stem().string() + ".prm")).string();
		BOOST_LOG_TRIVIAL(debug) << "Prm文件内容：\n" << prm_str;
		std::ofstream ofs(prm_path);
		if (ofs)
		{
		...
		// heg要求的Prm文件为Unix的，所以调用dos_2_unix转换一下
		const std::string dos2unix = current_path + "\\dos2unix.exe " + prm_path;
		...
		const std::string bat_path = (tmp_folder_path / (hp.input_file().stem().string() + ".bat")).string();
		ofs.open(bat_path);
		if (ofs)
		{
			ofs << bat_str;
