...
using namespace std;
std::string modis_api::Mrt_utils::load_template_string(const string& file_path)
{
	...
	cs output_projection_parameters, cs datum, double output_pixel_size)
{
	...
	try
	{
		...
		string mrt_prm_template_path = current_path + "//templates//MRT.tt";
		if (!fs::exists(mrt_prm_template_path))
			throw runtime_error(boost::str(boost::format("未找到MRT的Prm模板文件%1%") % mrt_prm_template_path));
		string mrt_prm_template = load_template_string(mrt_prm_template_path);
		string mrt_prm_str = str(boost::format(mrt_prm_template) % input_file_name %spatial_subset_type
		...
		//MRT .prm文件保存路径
		const string mrt_prm_path = temp_dir + fs::path(input_file_name).stem().string() + ".prm";
		BOOST_LOG_TRIVIAL(debug) << "Prm文件内容：\n" << mrt_prm_str;
		if (fs::exists(mrt_prm_path)) fs::remove(mrt_prm_path);
		ofstream ofs(mrt_prm_path);
		if (ofs)
			ofs << mrt_prm_str;
		ofs.clear();
		ofs.close();
		...
		string mrt_home = current_path + "\\MRT\\";
		string mrt_bat_template_path = current_path + "\\templates\\MRT_RUN.tt";
		if (!fs::exists(mrt_bat_template_path))
			throw runtime_error(boost::str(boost::format("未找到MRT的Bat模板文件%1%") % mrt_bat_template_path));
		string mrt_bat_template = load_template_string(mrt_bat_template_path);
		string mrt_bat_str = str(boost::format(mrt_bat_template) % mrt_home % mrt_prm_path);
		BOOST_LOG_TRIVIAL(debug) << "Bat文件内容：\n" << mrt_bat_str;
		const string mrt_bat_path = temp_dir + fs::path(input_file_name).stem().string() + ".bat";
		...
		string generated_tif_file_path = temp_dir + fs::path(input_file_name).stem().string() + "_mrt.LST_Day_1km.tif";
